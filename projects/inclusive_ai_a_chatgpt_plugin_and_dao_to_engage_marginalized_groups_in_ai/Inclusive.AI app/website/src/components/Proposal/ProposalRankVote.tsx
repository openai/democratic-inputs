...
export function ProposalRankVote(props: ProposalGenericProps) {
  const {
    choices,
    userVp,
  ...
  // sort choices by descending number of votes based on `userProposalVotes`
  const choiceRanks = useMemo(() => {
    return [...choices]
      .sort((a, b) => {
        const aVotes = getUserVoteChoice(userProposalVotes, a.index)
        const bVotes = getUserVoteChoice(userProposalVotes, b.index)
        return bVotes - aVotes
      })
      .reduce(
        (arr, choice, idx) => ({
          ...arr,
          [String(choice.index)]: idx,
        }),
        {} as Record<string, number>,
      )
  }, [choices, userProposalVotes])
