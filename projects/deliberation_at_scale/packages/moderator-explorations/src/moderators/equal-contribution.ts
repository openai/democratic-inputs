...
if (allContributed) {
  equalContributionCheck(messages);
}
...
const completion = await openai.chat.completions.create({
  ...
  messages: [
    ...
    Check if all participants are expressing their views equally. If an interruption is needed respond as a moderator. Otherwise say null.
    ...
    Reply in the following JSON format {message: "The interruption message."}
    ` },
    ...formattedMessages,
  ],
  model: 'gpt-4',
