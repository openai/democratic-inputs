import { pgTable, uuid, boolean, varchar, timestamp, pgEnum, integer, text, json, AnyPgColumn } from "drizzle-orm/pg-core";
...
export const crossPollinationType = pgEnum('crossPollinationType', ['outcome', 'topic']);
export const moderationType = pgEnum('moderationType', ['harrashment', 'spam', 'off_topic', 'other']);
export const completionType = pgEnum('completionType', ['gpt4']);
export const targetType = pgEnum('targetType', ['user', 'topic', 'room', 'participant', 'message', 'outcome', 'opinion', 'cross_pollination', 'completion', 'moderation']);
export const timingType = pgEnum('timingType', ['before_room', 'during_room', 'after_room', 'standalone']);
...
export const topics = pgTable(TOPICS_TABLE_NAME, {
  ...
  originalTopicId: uuid(ORIGINAL_TOPIC_ID_FIELD_NAME).references((): AnyPgColumn => topics.id),
...
export const rooms = pgTable(ROOMS_TABLE_NAME, {
  id: generateIdField(),
  active: generateActiveField(),
  topicId: uuid(TOPIC_ID_FIELD_NAME).notNull().references(() => topics.id),
...
export const participants = pgTable(PARTICIPANTS_TABLE_NAME, {
  id: generateIdField(),
  active: generateActiveField(),
  roomId: uuid(ROOM_ID_FIELD_NAME).notNull().references(() => rooms.id),
...
export const messages = pgTable(MESSAGES_TABLE_NAME, {
  ...
  originalMessageId: uuid(ORIGINAL_MESSAGE_ID_FIELD_NAME).references((): AnyPgColumn => messages.id),
...
export const outcomes = pgTable(OUTCOMES_TABLE_NAME, {
  ...
  originalOutcomeId: uuid(ORIGINAL_OUTCOME_ID_FIELD_NAME).references((): AnyPgColumn => outcomes.id),
  content: text('content').notNull().default(''),
  ...generateTimestampFields(),
});
...
export const outcomeSources = pgTable(OUTCOME_SOURCES_TABLE_NAME, {
  id: generateIdField(),
  outcomeId: uuid(OUTCOME_ID_FIELD_NAME).notNull().references(() => outcomes.id),
  messageId: uuid(MESSAGE_ID_FIELD_NAME).notNull().references(() => messages.id),
  ...generateTimestampFields(),
});
...
function generateTargetFields() {
  return {
    ...
    completionId: uuid(COMPLETION_ID_FIELD_NAME).references(() => completions.id),
    moderationId: uuid(MODERATION_ID_FIELD_NAME).references(() => moderations.id),
  };
};
